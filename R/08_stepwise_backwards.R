...
#'
#' @examples
stepwise_backward <- function(data, outcome_var,
  ...
  included_vars <- dep_vars
  excluded_vars <- c()

  #null model
  form_0 <- as.formula(paste(outcome_var, "~", paste(dep_vars, collapse = " + ") ))
  ...
  # condition_to_proceed_1 <- length(included_vars) == 0
  ...
  # #add variable names
  if(!"variable" %in% names(final_model_results)){
